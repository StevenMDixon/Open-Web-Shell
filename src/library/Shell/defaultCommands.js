...
function mapflags(helpFlags) {
  return helpFlags.reduce((acc, cur) => `${acc}${cur}\n`, '');
}

function mapOptions(options) {
  const l = Object.keys(options).reduce((acc, cur) => (`${acc}-${cur}, `), '');
  return `(${l})`;
}
  ...
  let list = [
    {
      ...
      func: (com) => {
        if (list.some(i => i.name === com)) {
          const selected = list.filter(item => item.name === com)[0];
          if (!selected.help) {
            return `No help available for Command: ${selected.name}`;
          }
          const flagHelp = Object.values(selected.help) || [];
          return `Command: ${com}\n
          ${`Description: ${selected.desc}` || ''}
          ${flagHelp.length > 0 ? mapflags(flagHelp) : ''}`;
        }
    ...
    {
      name: 'list',
      def: () => list.reduce((a, c) => { a.push(`${c.name} ${c.options ? mapOptions(c.options) : ''}`); return a; }, []),
      desc: 'Lists all of the commands that are accessible.',
      help: {
        t: '-t: a test flag used in development',
      },
    },
    {
      name: 'clear',
